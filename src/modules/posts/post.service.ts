...
class PostService {
  ...
  private readonly _redisService = redisService
    ...
    const { postId } = req.params
    const { allowComment, availability, content, tags, removeFiles, removeTags }: updatePostDto = req.body
...
if (tags?.length) {
    ...
    if (tags.length !== mentionsTags.length) {
        throw new AppError("some person you mentioned not found")
    }
...
post.tags = [...updateTags].map((id: string) => new Types.ObjectId(id))
  ...
  if (req.files?.length) {
    ...
    post.attachments?.push(...urls as string[]) 
}
